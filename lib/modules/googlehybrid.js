...
openidModule.submodule('googlehybrid')
    ...
    this.relyingParty =
      new oid.RelyingParty(this._myHostname + this._callbackPath, null, false, false, [
        ...
        , new oid.OAuthHybrid({
            ...
            , scope: 'function' === typeof this._scope ? this._scope : this._scope.join(' ')
          })
      ]);
  ...
  .sendToAuthenticationUri(function (req, res) {
    ...
    if(typeof(this._scope) === 'function') {
      this.relyingParty.extensions[1].requestParams['openid.oauth.scope'] = this._scope(req, res)
    }
